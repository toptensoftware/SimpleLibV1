...
void SaveWindowPlacement(HWND hWnd, const wchar_t* pszSection=L"Window", const wchar_t* pszPrefix=L"MainFrame", WINDOWPLACEMENT* pOverride=NULL);

// Loads the window placement.  Returns a ShowWindow constant for showing the window
int LoadWindowPlacement(HWND hWnd, const wchar_t* pszSection=L"Window", const wchar_t* pszPrefix=L"MainFrame");
