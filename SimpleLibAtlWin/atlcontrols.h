...
// Constructors
	...
	HWND Create(HWND hWndParent, RECT& rcPos, const wchar_t* szWindowName = NULL,
			DWORD dwStyle = 0, DWORD dwExStyle = 0,
			UINT nID = 0, LPVOID lpCreateParam = NULL)
	{
		return CWindow::Create(GetWndClassName(), hWndParent, rcPos, szWindowName, dwStyle, dwExStyle, nID, lpCreateParam);
	}
	HWND Create(HWND hWndParent, LPRECT lpRect = NULL, const wchar_t* szWindowName = NULL,
			DWORD dwStyle = 0, DWORD dwExStyle = 0,
...
// Attributes
	...
	{
		ATLASSERT(::IsWindow(m_hWnd));
		ATLASSERT(GetStyle() & (LBS_MULTIPLESEL|LBS_EXTENDEDSEL));
	...
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return SetItemData(nIndex, (LPARAM)(LPVOID)pData);
	}
	...
	int FindStringExact(int nIndexStart, const wchar_t* lpszFind) const
	{
		ATLASSERT(::IsWindow(m_hWnd));
...
// Operations
	...
	int Dir(UINT attr, const wchar_t* lpszWildCard)
	{
		ATLASSERT(::IsWindow(m_hWnd));
...
// Operations
	...
	// manipulating listbox items
//	int AddString(const wchar_t* lpszString)// Added For Unicode 
//	{
//		ATLASSERT(::IsWindow(m_hWnd));
...
// Operations
	...
	void InsertText(int nInsertAfterChar, const wchar_t* lpstrText, BOOL bNoScroll = FALSE, BOOL bCanUndo = FALSE)
	{
		SetSel(nInsertAfterChar, nInsertAfterChar, bNoScroll);
		ReplaceSel(lpstrText, bCanUndo);
	}
	void AppendText(const wchar_t* lpstrText, BOOL bNoScroll = FALSE, BOOL bCanUndo = FALSE)
	{
...
// Attributes
	...
	BOOL SetItemText(int nItem, int nSubItem, const wchar_t* lpszText)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	int InsertItem(int nItem, const wchar_t* lpszItem, int nImage)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	int GetStringWidth(const wchar_t* lpsz) const
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	BOOL SortItems(PFNLVCOMPARE pfnCompare, LPARAM dwData)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	int InsertColumn(int nCol, const wchar_t* lpszColumnHeading, int nFormat, int nWidth, int nSubItem)
	{
		LV_COLUMN column;
	...
	int InsertItem(UINT nMask, int nItem, const wchar_t* lpszItem, UINT nState, UINT nStateMask, int nImage, LPARAM lParam)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		LV_ITEM item;
		item.mask = nMask;
	...
	BOOL SetItem(int nItem, int nSubItem, UINT nMask, const wchar_t* lpszItem,
	...
	BOOL AddColumn(const wchar_t* strItem,int nItem,int nSubItem = -1,
	...
	BOOL AddItem(int nItem,int nSubItem,const wchar_t* strItem,int nImageIndex = -1)
	{
		LV_ITEM lvItem;
...
// Operations
	...
	HTREEITEM InsertItem(const wchar_t* lpszItem, int nImage,
	...
	HTREEITEM InsertItem(const wchar_t* lpszItem, HTREEITEM hParent, HTREEITEM hInsertAfter)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	BOOL SetItemText(HTREEITEM hItem, const wchar_t* lpszItem)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	BOOL SetItemData(HTREEITEM hItem, LPARAM dwData)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	BOOL SetItem(HTREEITEM hItem, UINT nMask, const wchar_t* lpszItem, int nImage,
	...
	HTREEITEM InsertItem(UINT nMask, const wchar_t* lpszItem, int nImage,
		int nSelectedImage, UINT nState, UINT nStateMask, LPARAM lParam,
...
// Attributes
	...
	LPARAM GetData();
	BOOL SetItem(UINT nMask, const wchar_t* lpszItem, int nImage, int nSelectedImage, UINT nState, UINT nStateMask, LPARAM lParam);
	BOOL SetText(const wchar_t* lpszItem);
	BOOL SetImage(int nImage, int nSelectedImage);
	BOOL SetState(UINT nState, UINT nStateMask);
	BOOL SetData(LPARAM dwData);

// Operations
	CTreeItem InsertAfter(const wchar_t* lpstrItem, HTREEITEM hItemAfter, int nImageIndex)
	{
		return _Insert(lpstrItem, nImageIndex, hItemAfter);
	}
	CTreeItem AddHead(const wchar_t* lpstrItem, int nImageIndex)
	{
		return _Insert(lpstrItem, nImageIndex, TVI_FIRST);
	}
	CTreeItem AddTail(const wchar_t* lpstrItem, int nImageIndex)
	{
	...
	CTreeItem _Insert(const wchar_t* lpstrItem, int nImageIndex, HTREEITEM hItemAfter);
	int GetImageIndex();
};
...
// Operations (overides that return CTreeItem)
	...
	CTreeItem InsertItem(const wchar_t* lpszItem, int nImage,
		int nSelectedImage, HTREEITEM hParent, HTREEITEM hInsertAfter)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE, lpszItem, nImage, nSelectedImage, 0, 0, 0, hParent, hInsertAfter);
	}
	CTreeItem InsertItem(const wchar_t* lpszItem, HTREEITEM hParent, HTREEITEM hInsertAfter)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return InsertItem(TVIF_TEXT, lpszItem, 0, 0, 0, 0, 0, hParent, hInsertAfter);
	}
	...
	CTreeItem InsertItem(UINT nMask, const wchar_t* lpszItem, int nImage,
		int nSelectedImage, UINT nState, UINT nStateMask, LPARAM lParam,
		HTREEITEM hParent, HTREEITEM hInsertAfter)
	{
		ATLASSERT(::IsWindow(m_hWnd));
...
inline LPARAM CTreeItem::GetData()
{
	ATLASSERT(m_pTreeView != NULL);
	return m_pTreeView->GetItemData(m_hTreeItem);
}
inline BOOL CTreeItem::SetItem(UINT nMask, const wchar_t* lpszItem, int nImage,
		int nSelectedImage, UINT nState, UINT nStateMask, LPARAM lParam)
{
	ATLASSERT(m_pTreeView != NULL);
	return m_pTreeView->SetItem(m_hTreeItem, nMask, lpszItem, nImage, nSelectedImage, nState, nStateMask, lParam);
}
inline BOOL CTreeItem::SetText(const wchar_t* lpszItem)
{
	ATLASSERT(m_pTreeView != NULL);
...
inline BOOL CTreeItem::SetData(LPARAM dwData)
{
	ATLASSERT(m_pTreeView != NULL);
...
inline CTreeItem CTreeItem::_Insert(const wchar_t* lpstrItem, int nImageIndex, HTREEITEM hItemAfter)
{
	ATLASSERT(m_pTreeView != NULL);
	TV_INSERTSTRUCT ins;
...
// Operations
	...
	void SaveState(HKEY hKeyRoot, const wchar_t* lpszSubKey, const wchar_t* lpszValueName)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		TBSAVEPARAMS tbs;
		tbs.hkr = hKeyRoot;
	...
	void RestoreState(HKEY hKeyRoot, const wchar_t* lpszSubKey, const wchar_t* lpszValueName)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		TBSAVEPARAMS tbs;
		tbs.hkr = hKeyRoot;
	...
	int AddStrings(const wchar_t* lpszStrings)
	{
		ATLASSERT(::IsWindow(m_hWnd));
...
// Attributes
	...
	BOOL SetText(int nPane, const wchar_t* lpszText, int nType = 0)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		ATLASSERT(nPane < 256);
...
// Operations
	...
	BOOL AddTool(HWND hWnd, const wchar_t* lpszText = LPSTR_TEXTCALLBACK, LPCRECT lpRectTool = NULL, UINT nIDTool = 0)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		ATLASSERT(hWnd != NULL);
	...
	void UpdateTipText(const wchar_t* lpszText, HWND hWnd, UINT nIDTool = 0)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		ATLASSERT(hWnd != NULL);
...
// Operations
	BOOL MakeDragList()
	{
		...
		if(uMsg == 0)
		{
/*?*/           ::EnterCriticalSection(&_AtlBaseModule.m_csWindowCreate);
			if(uMsg == 0)
				uMsg = ::RegisterWindowMessage(DRAGLISTMSGSTRING);
/*?*/           ::LeaveCriticalSection(&_AtlBaseModule.m_csWindowCreate);
		}
		ATLASSERT(uMsg != 0);
		return bRet;
	}
