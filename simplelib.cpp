...
// Constructor
template <class T, class TSem>
CThreadSafeRingBuffer<T,TSem>::CThreadSafeRingBuffer(int iCapacity)
{
	m_iCapacity=iCapacity;
	m_pMem=(T*)malloc(sizeof(T)*m_iCapacity);
	m_pWritePosA=m_pMem;
...
// Destructor
template <class T, class TSem>
CThreadSafeRingBuffer<T,TSem>::~CThreadSafeRingBuffer()
{
	RemoveAll();
	free(m_pMem);
}
...
template <class T, class TSem>
void CThreadSafeRingBuffer<T,TSem>::Reset(int iNewCapacity)
{
	RemoveAll();
	if (iNewCapacity && iNewCapacity!=m_iCapacity)
	{
...
template <class T, class TSem>
bool CThreadSafeRingBuffer<T,TSem>::IsEmpty() const
{
	return m_pReadPosB==m_pWritePosB;
}

template <class T, class TSem>
bool CThreadSafeRingBuffer<T,TSem>::IsFull() const
{
	return AdvancePtr(m_pWritePosA)==m_pReadPosB;
}

template <class T, class TSem>
bool CThreadSafeRingBuffer<T,TSem>::IsOverflow() const
{
	return m_bOverflow;
}

template <class T, class TSem>
bool CThreadSafeRingBuffer<T,TSem>::Enqueue(const T& t)
{
	// Advance A ptr
	T* pWrite;
	T* pNext;
	...
	{
		pWrite=m_pWritePosA;
		pNext=AdvancePtr(m_pWritePosA);

		// Check for overflow...
		if (pNext==m_pReadPosB)
		{
			m_bOverflow=true;
			return false;
		}

		if (CompareExchangePointer((void**)&m_pWritePosA, pNext, pWrite)==pWrite)
			break;
	}

	// Write it
	Constructor(pWrite, TSem::OnAdd(t, this));

	// Advance write pointer B
	while (CompareExchangePointer((void**)&m_pWritePosB, pNext, pWrite)!=pWrite);

	return true;
}

template <class T, class TSem>
bool CThreadSafeRingBuffer<T,TSem>::Dequeue(T& t)
{
	// Advance read ptr A
	T* pRead;
	T* pNext;
	...
	{
		pRead=m_pReadPosA;
		pNext=AdvancePtr(m_pReadPosA);
		if (pRead==m_pWritePosB)
			return false;
		if (CompareExchangePointer((void**)&m_pReadPosA, pNext, pRead)==pRead)
			break;
	}
	...
	// Advance read pointer B
	while (CompareExchangePointer((void**)&m_pReadPosB, pNext, pRead)!=pRead);
...
template <class T, class TSem>
void CThreadSafeRingBuffer<T,TSem>::RemoveAll()
{
	T t;
	while (Dequeue(t))
	{
...
template <class T, class TSem>
int CThreadSafeRingBuffer<T,TSem>::GetCapacity() const
{
	return m_iCapacity;
}
...
// Operations
template <class T, class TSem>
T* CThreadSafeRingBuffer<T,TSem>::AdvancePtr(T* p) const
{
	// Next position
	p++;
